         ...
         pattern="", group="", all.info=FALSE, sep="\t", path=NULL)
{
  ...
  # Environment found
    ...
    if(length(Items) == 0)
      if(all.info)
        return(invisible(data.frame(Name=character(), Dims=character(),
          Group=character(), Class=character(), Recusive=logical(),
    ...
    res <- data.frame(t(sapply(Items, describe, pos=envir, all.info=all.info)),
             stringsAsFactors=FALSE)
  ...
  if(is.null(path))
    ...
    write.table(res, file=path, row.names=FALSE, quote=FALSE, sep ="\t")
}
