...
function(x, type="raw", objname=deparse(substitute(x)), ...) {
  ...
  if(.Platform$OS == "windows")
    trExport(x=xexp, type=type, file="clipboard", append=FALSE,
             objname=objname, ...)
  else { ### Rem: according to a suggestion by Ted Harding... not tested yet!
    File <- tempfile()
    trExport(x=xexp, type=type, file=File, append=FALSE, objname=objname, ...)
    # TO DO: manage errors!
    system(paste("wxcopy <", File), wait=TRUE, invisible=TRUE)
    unlink(File)
  }
}
